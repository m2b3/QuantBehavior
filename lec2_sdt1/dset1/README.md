# Presaccadic attention data analysis

This repository includes a Marimo teaching notebook that reconstructs d-prime
and criterion from the deposited trial data of Hanning, Fernández & Carrasco
(2023).
...
marimo edit criterion_reconstruction.py
```

Open the local URL printed by Marimo (normally <http://127.0.0.1:2718>).

The notebook is linear and self-contained: it reads the trial CSVs directly,
reconstructs participant-first d-prime, and presents criterion as a new
descriptive analysis.

## Retained supporting tools

The earlier interactive dashboard is retained at
[`other/viewer.py`](other/viewer.py). Run it with
`marimo run other/viewer.py` if needed.

The Experiment 1 forensic reconciliation audit is retained at
[`other/reconcile_exp1.py`](other/reconcile_exp1.py), with its findings in
[`other/RECONCILIATION.md`](other/RECONCILIATION.md). It tests candidate
trial-exclusion rules against the deposited processed source values; it is not
part of the criterion notebook's runtime analysis.

The retained dashboard's publication explorer uses `public/source_data.json`, an exact,
...
## What the retained viewer contains
...
Its extreme-rate rule replaces rates of 0 and 1 with .01 and .99. The retained viewer also offers log-linear correction and no correction.

The deposited CSVs do not include a data dictionary. Cross-column consistency identifies the test stimulus and response codes exactly, but it does not name which orientation sign is counter-clockwise. The retained viewer defaults to `-1` as the signal and lets the user reverse that convention. Reversal flips criterion's sign.

The default **figure-aligned approximation** retains all Experiment 1 neutral trials and saccade trials whose deposited latency is below 350 ms. This empirically minimizes d-prime error against `fig2_psa.mat`; it is not a cutoff stated in the paper. It retains 22,930 rows, 251 more than the reported final count, because the CSVs do not encode the blink/fixation traces or target-relative landing errors required to reproduce every paper exclusion. Experiment 2 uses all deposited rows and reproduces its source d-primes exactly. Select **all deposited rows** in the retained viewer for an unfiltered sensitivity analysis.

See [other/RECONCILIATION.md](other/RECONCILIATION.md) for the candidate rules, validation metrics, participant-row provenance issue, and limitations.

Reproduce the reconciliation table with:

```powershell
python other/reconcile_exp1.py
```
